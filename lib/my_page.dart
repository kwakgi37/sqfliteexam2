...
class MyPage extends StatefulWidget {
  ...
  MyPage({required this.databaseHelper});
  @override
  _MyPageState createState() => _MyPageState();
}

class _MyPageState extends State<MyPage> {
  late Future<List<Proj>> projs;

  @override
  void initState() {
    super.initState();
    projs = widget.databaseHelper.getProjs();
  }
  ...
  Future<void> _editProjectName(BuildContext context, int index) async {
    ...
    if (newName != null && newName.isNotEmpty) {
      // 프로젝트의 이름을 새로운 값으로 변경
      project.name = newName;

      // 변경된 프로젝트를 프로바이더에 전달하여 데이터베이스 업데이트
